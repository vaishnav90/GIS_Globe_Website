#!/usr/bin/env python3
"""
Script to add sample gallery items to the database for demonstration.
"""
...
def add_sample_gallery_items():
    """Add sample gallery items to the database."""
    with app.app_context():
        # Get the first user (admin) to assign gallery items to
        ...
        sample_items = [
            ...
            {
                'title': 'Data Collection Field Work',
                'description': 'Team members conducting field data collection for GIS projects',
        ...
        if existing_items:
            print(f"Found {len(existing_items)} existing gallery items. Skipping sample data addition.")
            return
        ...
        print(f"Successfully added {len(sample_items)} sample gallery items to the database!")
